...
for engine in "${ENGINES[@]}"; do
  ...
  if [[ -n "$GREP_FILTER" ]]; then
    node "$ROOT_DIR/cli_tests/memory/run_memory_semantics_tests.js" --repeat 1 --grep "$GREP_FILTER"
  else
    node "$ROOT_DIR/cli_tests/memory/run_memory_semantics_tests.js" --repeat 1
  fi
done
