...
static void constructorFinalizer(JSRuntime *rt, JSValue val)
{
    ...
    ConstructorInfo *constructorInfo = (ConstructorInfo *)JS_GetOpaque(val, runtime->constructorClassId);
...
napi_status NAPICreateEnv(napi_env *env, napi_runtime runtime)
{
    ...
    *env = (NAPIEnvironment *)malloc(sizeof(struct NAPIEnvironment));
    ...
    JSValue nativeValueFunc = JS_NewCFunctionData(context, [](JSContext *ctx, JSValueConst thisVal, int argc, JSValueConst *argv, int magic, JSValue *funcData) -> JSValue
                                                  { return JS_NewInt32(ctx, 6); }, 0, 0, 1, &jsNativeEngine);
    ...
    JSValue FinalizeCallback = JS_NewCFunction(context, [](JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) -> JSValue
                                               {
        napi_env env = (napi_env)JS_GetRuntimeOpaque(JS_GetRuntime(ctx));
        JSValue heldValue = argv[0];
        if (!JS_IsUndefined(heldValue)) {
            ExternalInfo *info = (ExternalInfo *)JS_GetOpaqueUnsafe(heldValue);
            if (info != nullptr) {
                info->finalizeCallback(env, info->data, info->finalizeHint);
            }
        }
        return JS_UNDEFINED; }, "FinalizationRegistryCallback", 0);

    (*env)->finalizationRegistry = JS_CallConstructor(context, FinalizationRegistry, 1, &FinalizeCallback);
    ...
    JSValue func = JS_Eval((*env)->context, script, strlen(script), "<napi_script>", JS_EVAL_TYPE_GLOBAL);
...
napi_status napi_open_escapable_handle_scope(napi_env env, napi_escapable_handle_scope *result)
{
    ...
    {
        // If the scope pool is empty, allocate a new scope
        NAPIEscapableHandleScope *handleScope = (NAPIEscapableHandleScope *)malloc(sizeof(struct NAPIEscapableHandleScope));
        RETURN_STATUS_IF_FALSE(handleScope, napi_memory_error)
        *result = handleScope;
...
napi_status napi_close_escapable_handle_scope(napi_env env, napi_escapable_handle_scope escapableScope)
{
    CHECK_ARG(env)
    ...
    LIST_REMOVE(scope, node);
    LIST_INSERT_HEAD(&globalHandlePool.escapableScopeList, escapableScope, node); // Return the scope to the pool
...
static napi_status setWeak(napi_env env, napi_value value, napi_ref ref)
{
    ...
    CHECK_NAPI(napi_get_property(env, value, (napi_value)&env->referenceSymbolValue, &referenceValue))
    napi_valuetype valueType;
    CHECK_NAPI(napi_typeof(env, referenceValue, &valueType))
    RETURN_STATUS_IF_FALSE(valueType == napi_undefined || valueType == napi_external, napi_generic_failure)
    ...
    if (valueType == napi_undefined)
    {
        ...
        {
            napi_status status =
                napi_create_external(env, referenceInfo, reference_finalize, nullptr, &referenceValue);
            if (TRUTHY(status != napi_ok))
            {
                free(referenceInfo);
        ...
        CHECK_NAPI(napi_set_property(env, value, (napi_value)&env->referenceSymbolValue, referenceValue))
...
static napi_status clearWeak(napi_env env, napi_ref ref)
{
    ...
    CHECK_NAPI(napi_get_property(env, (napi_value)&ref->value, (napi_value)&env->referenceSymbolValue, &externalValue))
    struct WeakReference *referenceInfo;
    ...
    if (needDelete)
    {
        bool deleteResult;
        // 一定不会在这里触发 GC
        CHECK_NAPI(napi_delete_property(env, (napi_value)&ref->value, (napi_value)&env->referenceSymbolValue, &deleteResult))
                                             ...
                                             napi_value *result)
{
    ...
    if (finalize_cb)
    {
        ExternalBufferInfo *external_arraybuffer_info = (ExternalBufferInfo *)malloc(sizeof(ExternalBufferInfo));
        external_arraybuffer_info->finalize_cb = finalize_cb;
        external_arraybuffer_info->hint = finalize_hint;

        value = JS_NewArrayBuffer(env->context, (uint8_t *)external_data, byte_length, buffer_finalizer, external_arraybuffer_info, false);
    }
    else
    {
        value = JS_NewArrayBuffer(env->context, (uint8_t *)external_data, byte_length, nullptr, nullptr, false);
    }
                                        ...
                                        napi_value *result)
{
    ...
    if (finalize_cb)
    {
        ...
        external_arraybuffer_info->finalize_cb = finalize_cb;
        external_arraybuffer_info->hint = finalize_hint;
        value = JS_NewArrayBuffer(env->context, (uint8_t *)data, length, buffer_finalizer, external_arraybuffer_info, false);
    }
    else
    {
                                   ...
                                   napi_value *result)
{
    ...
    JSValue global = JS_GetGlobalObject(env->context);
    JSValue typedArrayConstructor = JS_GetPropertyStr(env->context, global, typedArrayClassNames[type]);
    JS_FreeValue(env->context, global);
                                      ...
                                      napi_value *result)
{
    ...
    JSValue value = JS_NewStringLen(env->context, str, (length == NAPI_AUTO_LENGTH) ? strlen(str) : length);
    return CreateScopedResult(env, value, result);
}
                                     ...
                                     napi_value *result)
{
    ...
    JSValue value = JS_NewString16(env->context, (uint16_t *)str, length == NAPI_AUTO_LENGTH ? (int)char16_t_length(str) : (int)length);
...
int napi_get_typedarray_type(napi_env env, napi_value typedarray)
{
    ...
    int typedArrayType = findIndex(typedArrayClassNames, sizeof(typedArrayClassNames) / sizeof(typedArrayClassNames[0]), cName);
...
napi_status napi_get_value_string_utf8(napi_env env, napi_value value, char *str, size_t length, size_t *result)
{
    CHECK_ARG(env)
    CHECK_ARG(value)
...
napi_status napi_set_property(napi_env env, napi_value object, napi_value key, napi_value value)
{
    ...
    int result = JS_SetProperty(env->context, jsObject, keyAtom, JS_DupValue(env->context, jsValue));
...
static inline void napi_set_property_descriptor(napi_env env, napi_value object, napi_property_descriptor descriptor)
{
    ...
    if (descriptor.value)
    {
        JS_DefinePropertyValue(env->context, jsValue, key, JS_DupValue(env->context, *((JSValue *)descriptor.value)), JS_PROP_C_W_E);
    }
    else if (descriptor.method)
    {
        ...
        if (function)
        {
            JS_DefinePropertyValue(env->context, jsValue, key, JS_DupValue(env->context, *((JSValue *)function)), napi_default_method);
        }
    }
    else if (descriptor.getter || descriptor.setter)
    {
        ...
        if (setter || getter)
        {
            JS_DefineProperty(env->context, jsValue, key, JS_UNDEFINED, getter ? *((JSValue *)getter) : JS_UNDEFINED, setter ? *((JSValue *)setter) : JS_UNDEFINED, flags);
        }
    }
                               ...
                               const napi_value *argv, napi_value *result)
{
    ...
    JSValue returnValue = JS_Call(env->context, jsFunction, *((JSValue *)thisValue), (int)argc, args);
                             ...
                             JSValue *funcData)
{
    ...
    FunctionInfo *functionInfo = (FunctionInfo *)JS_GetOpaque(funcData[0], env->runtime->functionClassId);
                                 ...
                                 napi_value *result)
{
    ...
    if (utf8name && strcmp(utf8name, "") != 0)
    {
        ...
        int returnStatus = JS_DefinePropertyValue(env->context, functionValue, env->atoms.name, JS_NewString(env->context, utf8name), JS_PROP_CONFIGURABLE);
                              ...
                              napi_value *result)
{
    ...
    JSValue returnValue = JS_CallConstructor(env->context, *((JSValue *)constructor), (int)argc, args);
...
static JSValue CallConstructor(JSContext *ctx, JSValueConst newTarget, int argc, JSValueConst *argv)
{
    ...
    ConstructorInfo *constructorInfo = (ConstructorInfo *)JS_GetOpaque(prototypeValue, runtime->constructorClassId);
    ...
    if (constructorInfo == nullptr)
    {
        ...
        {
            constructorInfo = (ConstructorInfo *)JS_GetOpaque(superPrototype, runtime->constructorClassId);
    ...
    if (TRUTHY(!constructorInfo || !constructorInfo->functionInfo.callback || !constructorInfo->classId))
    {
        assert(false);
    ...
    JSValue thisValue = JS_NewObjectProtoClass(env->context, prototypeValue, constructorInfo->classId);
                              ...
                              napi_value *result)
{
    ...
    JSValue constructorValue = JS_NewCFunction2(env->context, CallConstructor, utf8name, 0, JS_CFUNC_constructor, 0);
...
napi_status napi_wrap(napi_env env, napi_value jsObject, void *nativeObject, napi_finalize finalize_cb, void *finalize_hint,
                      napi_ref *result)
{
...
napi_status napi_unwrap(napi_env env, napi_value jsObject, void **result)
{
    ...
    {
        ExternalInfo *externalInfo = (ExternalInfo *)JS_GetOpaque(external, env->runtime->externalClassId);
        if (externalInfo)
        {
...
napi_status napi_resolve_deferred(napi_env env, napi_deferred deferred, napi_value resolution)
{
    ...
    JSValue jsResult = JS_Call(env->context, *((JSValue *)deferred->resolve), JS_UNDEFINED, 1, &value);
    JS_FreeValue(env->context, jsResult);
...
napi_status napi_reject_deferred(napi_env env, napi_deferred deferred, napi_value rejection)
{
    ...
    JSValue jsResult = JS_Call(env->context, *((JSValue *)deferred->reject), JS_UNDEFINED, 1, &value);
    JS_FreeValue(env->context, jsResult);
                            ...
                            bool *is_promise)
{
    ...
    bool result = false;
    JSValue constructor = JS_GetProperty(env->context, *((JSValue *)value), env->atoms.constructor);
                            ...
                            napi_value *result)
{
    ...
    if (JS_IsException(eval_result))
    {
        ...
        const char *stack = JS_ToCString(env->context, JS_GetPropertyStr(env->context, exception, "stack"));
...
napi_status NAPIFreeEnv(napi_env env)
{
    ...
    LIST_FOREACH_SAFE(escapableHandleScope, &globalHandlePool.escapableScopeList, node, tempEscapableHandleScope)
    {
    ...
    {
        env->instanceData->finalizeCallback(env, env->instanceData->data, env->instanceData->finalizeHint);
