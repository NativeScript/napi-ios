...
void ModuleInternal::Init(napi_env env, const std::string& baseDir) {
    ...
    if (MODULE_CLASS == nullptr) {
        MODULE_CLASS = jenv.FindClass("org/nativescript/runtime/napi/Module");
    ...
    napi_value source;
    napi_create_string_utf8(env, requireFactoryScript.c_str(), NAPI_AUTO_LENGTH, &source);
...
napi_value ModuleInternal::LoadModule(napi_env env, const std::string& modulePath, const std::string& moduleCacheKey) {
    ...
    if (Util::EndsWith(modulePath, ".js")) {
        ...
        napi_status status = napi_run_script(env, script, &moduleFunc);
        if (status != napi_ok) {
