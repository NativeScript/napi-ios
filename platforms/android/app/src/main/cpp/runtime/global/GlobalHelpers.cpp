...
napi_value GetSmartJSONStringifyFunction(napi_env env) {
    ...
    napi_value script;
    napi_status status = napi_run_script(env, source, "json_helper.js", &script);
    if (status != napi_ok) {
        return nullptr;
    }
...
std::string ns::JsonStringifyObject(napi_env env, napi_value value, bool handleCircularReferences) {
    if (value == nullptr) {
        return "";
    }
...
napi_value ns::JsonParseString(napi_env env, const std::string& value) {
    napi_value global;
    napi_value json;
    napi_value parse;
...
std::vector<ns::JsStacktraceFrame> ns::BuildStacktraceFrames(napi_env env, napi_value error, int size) {
    std::vector<ns::JsStacktraceFrame> frames;
...
void ns::GlobalHelpers::onDisposeEnv(napi_env env) {
