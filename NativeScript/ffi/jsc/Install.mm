Object CreateNativeApiJSC(Runtime& runtime, const NativeApiJSCConfig& config) {
  auto bridge = std::make_shared<NativeApiJSCBridge>(config);
  return Object::createFromHostObject(
      runtime, std::make_shared<NativeApiHostObject>(std::move(bridge)));
}

void NativeApiJSCWriteSmokeStage(const char* stage) {
...
void InstallNativeApiJSCGlobalSymbols(Runtime& runtime, const char* globalName) {
  NativeApiJSCWriteSmokeStage("engine:globals:before-eval");
...
(function(nativeApiGlobalName) {
  ...
  runtime.evaluateJavaScript(std::make_shared<StringBuffer>(std::move(script)),
                             "NativeApiJSCGlobals.js");
  NativeApiJSCWriteSmokeStage("engine:globals:after-eval");
}

void InstallNativeApiJSC(Runtime& runtime, const NativeApiJSCConfig& config) {
  const char* globalName = config.globalName != nullptr && config.globalName[0] != '\0'
                               ? config.globalName
                               : "__nativeScriptNativeApi";
  NativeApiJSCWriteSmokeStage("engine:create-api");
  Object api = CreateNativeApiJSC(runtime, config);
  Object global = runtime.global();
  NativeApiJSCWriteSmokeStage("engine:set-global");
  global.setProperty(runtime, globalName, api);

  NativeApiJSCWriteSmokeStage("engine:set-interop");
  ...
  if (config.installGlobalSymbols) {
    NativeApiJSCWriteSmokeStage("engine:install-globals");
    InstallNativeApiJSCGlobalSymbols(runtime, globalName);
  } else {
    NativeApiJSCWriteSmokeStage("engine:install-aggregate-globals");
    InstallAggregateGlobals(runtime, api, "protocolNames");
  }
  NativeApiJSCWriteSmokeStage("engine:installed");
}
