bool isObjectiveCObjectType(const NativeApiJSCType& type) {
  switch (type.kind) {
...
class NativeApiJSCRuntimeScope final {
 public:
  explicit NativeApiJSCRuntimeScope(Runtime&) {}
};
#endif

struct NativeApiJSCSignature {
  ffi_cif cif = {};
  NativeApiJSCType returnType;
  std::vector<NativeApiJSCType> argumentTypes;
...
enum class NativeApiJSCCallbackThreadPolicy {
  Default,
  UI,
  JS,
};

NativeApiJSCCallbackThreadPolicy readEngineCallbackThreadPolicy(
  ...
  try {
    if (!functionObject.hasProperty(runtime, propertyName)) {
      return NativeApiJSCCallbackThreadPolicy::Default;
    }
    ...
    if (policy == "ui") {
      return NativeApiJSCCallbackThreadPolicy::UI;
    }
    if (policy == "js") {
      return NativeApiJSCCallbackThreadPolicy::JS;
    }
...
bool isNSErrorOutEngineMethodSignature(const NativeApiJSCSignature& signature) {
...
bool isNSErrorOutEngineMethodCallback(const NativeApiJSCSignature& signature) {
...
class NativeApiJSCArgumentFrame {
 public:
  explicit NativeApiJSCArgumentFrame(size_t count) : count_(count) {
  ...
  void rememberRoundTripValue(
      const std::shared_ptr<NativeApiJSCBridge>& bridge, Runtime& runtime,
...
Value wrapNativeFunctionPointer(Runtime& runtime,
                                const std::shared_ptr<NativeApiJSCBridge>& bridge,
                                const NativeApiJSCType& type, void* pointer,
                                bool block);

bool isObjectiveCObjectType(const NativeApiJSCType& type);

struct NativeApiJSCBlockDescriptor {
...
struct NativeApiJSCBlockLiteral {
  void* isa = nullptr;
  int flags = 0;
  int reserved = 0;
  void* invoke = nullptr;
  NativeApiJSCBlockDescriptor* descriptor = nullptr;
  void* callback = nullptr;
};

constexpr int kNativeApiJSCBlockNeedsFree = (1 << 24);
constexpr int kNativeApiJSCBlockHasCopyDispose = (1 << 25);
constexpr int kNativeApiJSCBlockRefCountOne = (1 << 1);
constexpr int kNativeApiJSCBlockHasSignature = (1 << 30);
...
std::string objcEncodingForEngineType(const NativeApiJSCType& type) {
  switch (type.kind) {
...
std::string objcBlockSignatureForEngineSignature(
    const NativeApiJSCSignature& signature) {
...
[[noreturn]] void throwNativeApiJSCCallbackException(
...
class NativeApiJSCCallback final
    : public std::enable_shared_from_this<NativeApiJSCCallback> {
 public:
  NativeApiJSCCallback(Runtime& runtime,
                       std::shared_ptr<NativeApiJSCBridge> bridge,
                       std::shared_ptr<NativeApiJSCSignature> signature,
                       Function function, bool block,
                       NativeApiJSCCallbackThreadPolicy threadPolicy =
                           NativeApiJSCCallbackThreadPolicy::Default,
                       bool bindThis = false)
      : runtimeOwner_(retainNativeApiJSCRuntime(runtime)),
    ...
    if (closure_ == nullptr || executable_ == nullptr ||
      ...
      throw JSError(runtime,
                                   "Unable to allocate native JSC callback.");
    }
    ...
    if (status != FFI_OK) {
      ...
      executable_ = nullptr;
      throw JSError(runtime,
                                   "Unable to prepare native JSC callback.");
    }

    if (block_) {
      blockSignature_ = objcBlockSignatureForEngineSignature(*signature_);
      descriptor_ = std::make_unique<NativeApiJSCBlockDescriptor>();
      descriptor_->reserved = 0;
      descriptor_->size = sizeof(NativeApiJSCBlockLiteral);
      ...
      blockLiteral_ = static_cast<NativeApiJSCBlockLiteral*>(
          calloc(1, sizeof(NativeApiJSCBlockLiteral)));
      if (blockLiteral_ == nullptr) {
        throw JSError(runtime, "Unable to allocate native JSC block callback.");
      }
      ...
      blockLiteral_->flags = kNativeApiJSCBlockNeedsFree |
                             kNativeApiJSCBlockHasCopyDispose |
                             kNativeApiJSCBlockRefCountOne |
                             kNativeApiJSCBlockHasSignature;
  ...
  const NativeApiJSCSignature& signature() const { return *signature_; }

  void retainInitialBlockLifetime(
      std::shared_ptr<NativeApiJSCCallback> lifetime) {
    if (block_) {
  ...
  bool releaseBlockCopy(const void* blockPointer) {
    ...
    std::shared_ptr<NativeApiJSCCallback> keepAlive;
    try {
  ...
  void invoke(void* ret, void* args[]) {
    if (runtime_ == nullptr || function_ == nullptr || signature_ == nullptr) {
      throwNativeApiJSCCallbackException("Invalid JSC callback.");
    }
    ...
    if (threadPolicy_ == NativeApiJSCCallbackThreadPolicy::UI) {
      callOnUIThread();
      if (!error.empty()) {
        if (!recordNativeCallbackException(error)) {
          throwNativeApiJSCCallbackException(error);
        }
      }
      return;
    }
    if (threadPolicy_ == NativeApiJSCCallbackThreadPolicy::JS) {
      callOnJSThread();
      if (!error.empty()) {
        if (!recordNativeCallbackException(error)) {
          throwNativeApiJSCCallbackException(error);
 ...
 private:
  void invokeOnCurrentThread(void* ret, void* args[], std::string* error) {
    try {
      NativeApiJSCRuntimeScope runtimeScope(*runtime_);
      ...
      if (isNSErrorOutEngineMethodCallback(*signature_)) {
        zeroReturnValue(ret);
        populateNSErrorOutArgument(args, "Unknown exception in native JSC callback.");
        return;
      }
      if (error != nullptr) {
        *error = "Unknown exception in native JSC callback.";
      }
      zeroReturnValue(ret);
    }
  }
  ...
  std::shared_ptr<NativeApiJSCSignature> signature_;
  std::shared_ptr<Function> function_;
  bool block_ = false;
  NativeApiJSCCallbackThreadPolicy threadPolicy_ =
      NativeApiJSCCallbackThreadPolicy::Default;
  bool bindThis_ = false;
  ffi_closure* closure_ = nullptr;
  void* executable_ = nullptr;
  std::string blockSignature_;
  std::unique_ptr<NativeApiJSCBlockDescriptor> descriptor_;
  NativeApiJSCBlockLiteral* blockLiteral_ = nullptr;
  std::shared_ptr<NativeApiJSCCallback> initialBlockLifetime_;
...
void nativeApiEngineBlockCopy(void* dst, void* src) {
  auto* dstBlock = static_cast<NativeApiJSCBlockLiteral*>(dst);
  auto* srcBlock = static_cast<NativeApiJSCBlockLiteral*>(src);
  if (dstBlock == nullptr || srcBlock == nullptr ||
      srcBlock->callback == nullptr) {
    return;
  }
  dstBlock->callback = srcBlock->callback;
  static_cast<NativeApiJSCCallback*>(srcBlock->callback)
      ->retainBlockCopy(dstBlock);
}

void nativeApiEngineBlockDispose(void* src) {
  auto* block = static_cast<NativeApiJSCBlockLiteral*>(src);
  if (block == nullptr || block->callback == nullptr) {
    return;
  }
  bool released =
      static_cast<NativeApiJSCCallback*>(block->callback)->releaseBlockCopy(block);
  if (released) {
...
void nativeApiEngineCallbackTrampoline(ffi_cif*, void* ret, void* args[],
                                    void* data) {
  auto callback = static_cast<NativeApiJSCCallback*>(data);
  if (callback == nullptr) {
    return;
  }
  @try {
    ...
    std::string message = description != nullptr
                              ? description
                              : "Objective-C exception in native JSC callback.";
...
NativeApiJSCType parseMetadataEngineType(MDMetadataReader* metadata,
                                      MDSectionOffset* offset,
                                      NativeApiJSCBridge* bridge) {
  ...
  *offset += sizeof(MDTypeKind);
  ...
  switch (kind) {
    case metagen::mdTypeArray: {
      ...
      *offset += sizeof(uint16_t);
      type.elementType =
          std::make_shared<NativeApiJSCType>(
              parseMetadataEngineType(metadata, offset, bridge));
      auto ffiOwner = std::make_shared<NativeApiJSCFfiType>();
...
std::shared_ptr<NativeApiJSCAggregateInfo> NativeApiJSCBridge::aggregateInfoFor(
  ...
  auto info = std::make_shared<NativeApiJSCAggregateInfo>();
  ...
  while (next) {
    ...
    NativeApiJSCAggregateField field;
...
ffi_type* ffiTypeForEngineArgument(const NativeApiJSCType& type) {
  switch (type.kind) {
    case metagen::mdTypeArray:
      return &ffi_type_pointer;
    default:
...
std::optional<NativeApiJSCSignature> parseMetadataEngineSignature(
    MDMetadataReader* metadata, MDSectionOffset signatureOffset,
    unsigned int implicitArgumentCount, NativeApiJSCBridge* bridge,
...
const NativeApiSymbol* findObjCAggregateSymbol(
    NativeApiJSCBridge* bridge, const std::string& name, bool isUnion) {
...
NativeApiJSCType parseObjCEncodedEngineType(
    const char* encoding, NativeApiJSCBridge* bridge = nullptr,
    const char** endEncoding = nullptr);

bool unsupportedEngineType(const NativeApiJSCType& type);

NativeApiJSCType parseObjCEncodedAggregateEngineType(
    const char* encoding, NativeApiJSCBridge* bridge, const char** endEncoding) {
  NativeApiJSCType type;
...
NativeApiJSCType parseObjCEncodedArrayEngineType(
    const char* encoding, NativeApiJSCBridge* bridge, const char** endEncoding) {
  NativeApiJSCType type;
  ...
  type.elementType = std::make_shared<NativeApiJSCType>(
...
std::optional<NativeApiJSCSignature> parseObjCMethodEngineSignature(
    Method method, NativeApiJSCBridge* bridge = nullptr) {
  if (method == nullptr) {
    return std::nullopt;
  }
...
bool prepareEngineMethodSignature(NativeApiJSCSignature* signature) {
...
bool isRuntimeAggregateType(const NativeApiJSCType& type) {
  switch (type.kind) {
    case metagen::mdTypeStruct:
    case metagen::mdTypeArray:
    case metagen::mdTypeVector:
...
bool reconcileObjCMethodRuntimeType(NativeApiJSCType* metadataType,
                                    const NativeApiJSCType& runtimeType,
                                    bool* abiChanged) {
...
bool reconcileObjCMethodRuntimeSignature(NativeApiJSCSignature* signature,
                                         const NativeApiJSCSignature& runtime) {
  if (signature == nullptr ||
...
bool signatureSupportedForEngineCallback(const NativeApiJSCSignature& signature) {
...
std::shared_ptr<NativeApiJSCCallback> createEngineCallback(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
    const NativeApiJSCType& type, Function function, bool block,
    NativeApiJSCCallbackThreadPolicy threadPolicy =
        NativeApiJSCCallbackThreadPolicy::Default) {
  ...
  auto signature =
      std::make_shared<NativeApiJSCSignature>(std::move(*parsed));
  auto callback = std::make_shared<NativeApiJSCCallback>(
...
std::shared_ptr<NativeApiJSCCallback> createEngineMethodCallback(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
...
std::shared_ptr<NativeApiJSCCallback> createEngineMethodCallback(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
    const std::string& selectorName, NativeApiJSCSignature signature,
    Function function) {
  ...
  auto sharedSignature =
      std::make_shared<NativeApiJSCSignature>(std::move(signature));
  auto threadPolicy = readEngineCallbackThreadPolicy(runtime, function);
  auto callback = std::make_shared<NativeApiJSCCallback>(
