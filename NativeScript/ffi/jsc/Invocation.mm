...
class NativeApiJSCReturnStorage {
 public:
  explicit NativeApiJSCReturnStorage(size_t size)
...
class NativeApiJSCPointerFrame {
 public:
  explicit NativeApiJSCPointerFrame(size_t count) : count_(count) {
...
Value constantToValue(Runtime& runtime,
  ...
  switch (evalKind) {
    ...
    case metagen::mdEvalString: {
      ...
      NativeApiJSCType stringObjectType;
  ...
  NativeApiJSCType type = parseMetadataEngineType(metadata, &typeOffset, bridge.get());
...
void prepareEngineArgument(Runtime& runtime,
                        const std::shared_ptr<NativeApiJSCBridge>& bridge,
                        const NativeApiJSCType& type, const Value& arg,
                        size_t index, NativeApiJSCArgumentFrame& frame) {
...
inline uint64_t dispatchIdForEngineSignature(
    const NativeApiJSCSignature& signature, SignatureCallKind kind) {
...
bool tryCallFastEngineCFunction(
    ...
    void* function, const NativeApiJSCSignature& signature, const Value* args,
    size_t count, Value* result);

Value callNativeFunctionPointer(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
    const NativeApiJSCType& type, void* pointer, bool block, const Value* args,
    size_t count) {
  if (pointer == nullptr) {
  ...
  NativeApiJSCPointerFrame values(signature->argumentTypes.size() + 1);
  if (block) {
  ...
  if (block) {
    auto literal = static_cast<NativeApiJSCBlockLiteral*>(pointer);
...
Value wrapNativeFunctionPointer(Runtime& runtime,
                                ...
                                bool block) {
  const char* functionName = block ? "NativeApiJSCBlock" : "NativeApiJSCFunctionPointer";
...
Value callCFunction(Runtime& runtime,
  ...
  NativeApiJSCSignature& signature = prepared->signature;
  Value fastResult;
...
Value callCFunction(Runtime& runtime,
                    ...
                    size_t count) {
  auto prepared = std::make_shared<NativeApiJSCPreparedCFunctionInvocation>();
  prepared->symbol = symbol;
  return callCFunction(runtime, bridge, prepared, args, count);
}

bool signatureSupportedForEngineInvocation(
    const std::optional<NativeApiJSCSignature>& signature) {
  if (!signature || !signature->prepared || signature->variadic ||
      unsupportedEngineType(signature->returnType)) {
    return false;
  }
...
bool isFastEngineObjectType(const NativeApiJSCType& type) {
  switch (type.kind) {
...
bool isFastEngineSignedIntegerType(const NativeApiJSCType& type) {
  switch (type.kind) {
...
bool isFastEngineUnsignedIntegerType(const NativeApiJSCType& type) {
  switch (type.kind) {
...
bool readFastEngineObjectArgument(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
    const NativeApiJSCType& type, const Value& value,
    NativeApiJSCArgumentFrame& frame, id* result) {
  if (result == nullptr) {
    return false;
  }
...
Value callFastEngineCFunctionWithReturn(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
    void* function, NativeApiJSCType returnType, Args... nativeArgs) {
  bool dispatchingNativeCallToUI = shouldDispatchNativeCallToUI();
  bool retainedReturn = false;

  auto finalizeObjectReturn = [&](id object) -> Value {
    NativeApiJSCType effectiveReturnType = returnType;
...
bool isFastEngineCallableReturnType(const NativeApiJSCType& returnType) {
...
bool tryCallFastEngineCFunction(
  ...
  NativeApiJSCArgumentFrame frame(count);
...
Value callFastEngineObjCWithReturn(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
    id receiver, SEL selector, NativeApiJSCType returnType,
...
Value callFastEngineObjC1(
    Runtime& runtime, const std::shared_ptr<NativeApiJSCBridge>& bridge,
    id receiver, SEL selector, const NativeApiJSCType& returnType,
...
prepareNativeApiJSCObjCInvocation(
  ...
  std::optional<NativeApiJSCSignature> signature;
  std::optional<NativeApiJSCSignature> runtimeSignature;
  if (member != nullptr &&
  ...
  auto prepared = std::make_shared<NativeApiJSCPreparedObjCInvocation>();
...
Value callObjCSelector(Runtime& runtime,
  ...
  NativeApiJSCPreparedObjCInvocation engineInvocation;
