...
inline bool isV8DirectObjectKind(MDTypeKind kind) {
  switch (kind) {
    case mdTypeAnyObject:
    case mdTypeProtocolObject:
    case mdTypeClassObject:
...
inline bool isV8DirectIntegerKind(MDTypeKind kind) {
  switch (kind) {
    case mdTypeChar:
    case mdTypeSInt:
    case mdTypeSShort:
...
bool tryConvertV8NSUIntegerArgument(napi_env env, v8::Local<v8::Value> value,
                                    NSUInteger* result) {
  if (env == nullptr || result == nullptr || value.IsEmpty()) {
    return false;
  }
  ...
  if (value->IsBigInt()) {
    bool lossless = false;
    *result = static_cast<NSUInteger>(value.As<v8::BigInt>()->Uint64Value(&lossless));
    return true;
  }
...
bool tryInvokeObjCV8DirectFastPath(napi_env env,
                                   ...
                                   Cif* cif, id self) {
  if (env == nullptr || descriptor == nullptr || cif == nullptr || self == nil ||
      cif->returnType == nullptr) {
    return false;
  }

  v8::Isolate* isolate = info.GetIsolate();
  const MDTypeKind returnKind = cif->returnType->kind;

  @try {
  if (cif->argc == 0) {
    switch (returnKind) {
      case mdTypeBool: {
        BOOL value = reinterpret_cast<BOOL (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Boolean::New(isolate, value != NO));
        return true;
      }
      case mdTypeChar: {
        int8_t value =
            reinterpret_cast<int8_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Integer::New(isolate, value));
        return true;
      }
      case mdTypeUChar:
      case mdTypeUInt8: {
        uint8_t value =
            reinterpret_cast<uint8_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Integer::NewFromUnsigned(isolate, value));
        return true;
      }
      case mdTypeSShort: {
        int16_t value =
            reinterpret_cast<int16_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Integer::New(isolate, value));
        return true;
      }
      case mdTypeUShort: {
        uint16_t value =
            reinterpret_cast<uint16_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Integer::NewFromUnsigned(isolate, value));
        return true;
      }
      case mdTypeSInt: {
        int32_t value =
            reinterpret_cast<int32_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Integer::New(isolate, value));
        return true;
      }
      case mdTypeUInt: {
        uint32_t value =
            reinterpret_cast<uint32_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Integer::NewFromUnsigned(isolate, value));
        return true;
      }
      case mdTypeSLong:
      case mdTypeSInt64: {
        int64_t value =
            reinterpret_cast<int64_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        constexpr int64_t kMaxSafeInteger = 9007199254740991LL;
        if (value > kMaxSafeInteger || value < -kMaxSafeInteger) {
          info.GetReturnValue().Set(v8::BigInt::New(isolate, value));
        } else {
      ...
      case mdTypeUInt64: {
        uint64_t value =
            reinterpret_cast<uint64_t (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        constexpr uint64_t kMaxSafeInteger = 9007199254740991ULL;
        if (value > kMaxSafeInteger) {
          info.GetReturnValue().Set(v8::BigInt::NewFromUnsigned(isolate, value));
        } else {
      ...
      case mdTypeFloat: {
        float value =
            reinterpret_cast<float (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Number::New(isolate, value));
        return true;
      }
      case mdTypeDouble: {
        double value =
            reinterpret_cast<double (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        info.GetReturnValue().Set(v8::Number::New(isolate, value));
        return true;
      }
      case mdTypeNSStringObject: {
        NSString* value =
            reinterpret_cast<NSString* (*)(id, SEL)>(objc_msgSend)(self, descriptor->selector);
        v8::Local<v8::Value> result;
        if (!TryFastConvertV8NSStringReturnValue(env, &value, &result)) {
          return false;
        }
  ...
  if (returnKind == mdTypeBool && cif->argc == 1 &&
      static_cast<unsigned int>(info.Length()) >= 1) {
    const MDTypeKind argKind = cif->argTypes[0]->kind;
    if (argKind == mdTypeSelector) {
      SEL selector = nullptr;
      if (!TryFastConvertV8SelectorArgument(env, info[0], &selector)) {
        return false;
      }
      BOOL value = reinterpret_cast<BOOL (*)(id, SEL, SEL)>(objc_msgSend)(
          self, descriptor->selector, selector);
      info.GetReturnValue().Set(v8::Boolean::New(isolate, value != NO));
      return true;
    }

    if (argKind == mdTypeClass || argKind == mdTypeClassObject) {
      Class cls = Nil;
      if (!TryFastUnwrapV8ClassArgument(env, info[0], &cls)) {
        return false;
      }
      BOOL value = reinterpret_cast<BOOL (*)(id, SEL, Class)>(objc_msgSend)(
          self, descriptor->selector, cls);
      info.GetReturnValue().Set(v8::Boolean::New(isolate, value != NO));
      return true;
    }
  }

  if ((returnKind == mdTypeAnyObject || returnKind == mdTypeProtocolObject ||
       returnKind == mdTypeClassObject) &&
      cif->argc == 1 && static_cast<unsigned int>(info.Length()) >= 1) {
    const MDTypeKind argKind = cif->argTypes[0]->kind;
    if (isV8DirectIntegerKind(argKind)) {
      NSUInteger arg0 = 0;
      if (!tryConvertV8NSUIntegerArgument(env, info[0], &arg0)) {
        return false;
      }
      id value = reinterpret_cast<id (*)(id, SEL, NSUInteger)>(objc_msgSend)(
          self, descriptor->selector, arg0);
  ...
  if (returnKind == mdTypeVoid) {
    if (cif->argc == 1 && static_cast<unsigned int>(info.Length()) >= 1) {
      ...
      if (isV8DirectObjectKind(argKind)) {
        ...
        reinterpret_cast<void (*)(id, SEL, id)>(objc_msgSend)(self, descriptor->selector, arg0);
    ...
    if (cif->argc == 2 && static_cast<unsigned int>(info.Length()) >= 2) {
      const MDTypeKind arg0Kind = cif->argTypes[0]->kind;
      const MDTypeKind arg1Kind = cif->argTypes[1]->kind;
      if (isV8DirectObjectKind(arg0Kind) && isV8DirectObjectKind(arg1Kind)) {
        id arg0 = nil;
        id arg1 = nil;
        if (!TryFastUnwrapV8ObjectArgument(env, info[0], &arg0) ||
            !TryFastUnwrapV8ObjectArgument(env, info[1], &arg1)) {
          return false;
        }
        reinterpret_cast<void (*)(id, SEL, id, id)>(objc_msgSend)(
            self, descriptor->selector, arg0, arg1);
        info.GetReturnValue().Set(v8::Undefined(isolate));
        return true;
      }

      if (isV8DirectObjectKind(arg0Kind) && isV8DirectIntegerKind(arg1Kind)) {
        id arg0 = nil;
        NSUInteger arg1 = 0;
        if (!TryFastUnwrapV8ObjectArgument(env, info[0], &arg0) ||
            !tryConvertV8NSUIntegerArgument(env, info[1], &arg1)) {
          return false;
        }
        reinterpret_cast<void (*)(id, SEL, id, NSUInteger)>(objc_msgSend)(
            self, descriptor->selector, arg0, arg1);
