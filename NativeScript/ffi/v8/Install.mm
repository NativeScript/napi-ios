Object CreateNativeApiV8(Runtime& runtime, const NativeApiV8Config& config) {
  auto bridge = std::make_shared<NativeApiV8Bridge>(config);
  return Object::createFromHostObject(runtime,
                                      std::make_shared<NativeApiHostObject>(std::move(bridge)));
}

void NativeApiV8WriteSmokeStage(const char* stage) {
...
void InstallNativeApiV8GlobalSymbols(Runtime& runtime, const char* globalName) {
  NativeApiV8WriteSmokeStage("engine:globals:before-eval");
...
(function(nativeApiGlobalName) {
  ...
  runtime.evaluateJavaScript(std::make_shared<StringBuffer>(std::move(script)),
                             "NativeApiV8Globals.js");
  NativeApiV8WriteSmokeStage("engine:globals:after-eval");
}

void InstallNativeApiV8(Runtime& runtime, const NativeApiV8Config& config) {
  const char* globalName = config.globalName != nullptr && config.globalName[0] != '\0'
                               ? config.globalName
                               : "__nativeScriptNativeApi";
  NativeApiV8WriteSmokeStage("engine:create-api");
  Object api = CreateNativeApiV8(runtime, config);
  Object global = runtime.global();
  NativeApiV8WriteSmokeStage("engine:set-global");
  global.setProperty(runtime, globalName, api);

  NativeApiV8WriteSmokeStage("engine:set-interop");
  ...
  if (config.installGlobalSymbols) {
    NativeApiV8WriteSmokeStage("engine:install-globals");
    InstallNativeApiV8GlobalSymbols(runtime, globalName);
  } else {
    NativeApiV8WriteSmokeStage("engine:install-aggregate-globals");
    InstallAggregateGlobals(runtime, api, "protocolNames");
  }
  NativeApiV8WriteSmokeStage("engine:installed");
}
