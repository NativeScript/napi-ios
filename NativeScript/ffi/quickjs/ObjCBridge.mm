...
static char gNativeApiQuickJSExtendedClassKey;

void markNativeApiQuickJSExtendedClass(Class cls) {
  if (cls == Nil) {
    return;
  }
  objc_setAssociatedObject(cls, &gNativeApiQuickJSExtendedClassKey, @YES,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

bool isNativeApiQuickJSExtendedClass(Class cls) {
  Class current = cls;
  while (current != Nil) {
    if (objc_getAssociatedObject(current, &gNativeApiQuickJSExtendedClassKey) != nil) {
      return true;
    }
...
struct NativeApiQuickJSAggregateInfo;

struct NativeApiQuickJSFfiType {
  ffi_type type = {};
  std::vector<ffi_type*> elements;

  NativeApiQuickJSFfiType() {
...
struct NativeApiQuickJSType {
  ...
  std::shared_ptr<NativeApiQuickJSType> elementType;
  std::shared_ptr<NativeApiQuickJSAggregateInfo> aggregateInfo;
  std::shared_ptr<NativeApiQuickJSFfiType> ownedFfiType;
};

struct NativeApiQuickJSAggregateField {
  std::string name;
  uint16_t offset = 0;
...
struct NativeApiQuickJSAggregateInfo {
  ...
  std::vector<NativeApiQuickJSAggregateField> fields;
  std::shared_ptr<NativeApiQuickJSFfiType> ffi;
};
...
class NativeApiQuickJSBridge {
 public:
  explicit NativeApiQuickJSBridge(const NativeApiQuickJSConfig& config)
  ...
  std::shared_ptr<NativeApiQuickJSScheduler> scheduler() const { return scheduler_; }
  ...
  std::shared_ptr<NativeApiQuickJSAggregateInfo> aggregateInfoFor(
 ...
 private:
  ...
  std::shared_ptr<NativeApiQuickJSScheduler> scheduler_;
  ...
  std::unordered_map<MDSectionOffset, std::shared_ptr<NativeApiQuickJSAggregateInfo>>
...
NativeApiQuickJSType nativeObjectReturnType(
...
NativeApiQuickJSType nativeObjectReturnTypeForClass(Class cls) {
  if (cls != Nil) {
