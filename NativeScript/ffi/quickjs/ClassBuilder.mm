...
struct NativeApiQuickJSClassBuilderRegistration {
  std::shared_ptr<Runtime> runtimeOwner;
  Runtime* runtime = nullptr;
  std::shared_ptr<NativeApiQuickJSBridge> bridge;
};

std::mutex gNativeApiQuickJSClassBuilderMutex;
std::unordered_map<Class, NativeApiQuickJSClassBuilderRegistration>
    gNativeApiQuickJSClassBuilders;
struct NativeApiQuickJSKnownExposedMethod {
  std::string selectorName;
  NativeApiQuickJSSignature signature;
};
std::mutex gNativeApiQuickJSKnownExposedMethodsMutex;
std::unordered_map<std::string, NativeApiQuickJSKnownExposedMethod>
    gNativeApiQuickJSKnownExposedMethods;

void rememberNativeApiQuickJSClassBuilder(
    Runtime& runtime, const std::shared_ptr<NativeApiQuickJSBridge>& bridge,
    Class cls) {
  if (cls == Nil) {
    return;
  }
  std::lock_guard<std::mutex> lock(gNativeApiQuickJSClassBuilderMutex);
  auto runtimeOwner = retainNativeApiQuickJSRuntime(runtime);
  gNativeApiQuickJSClassBuilders[cls] = NativeApiQuickJSClassBuilderRegistration{
...
void rememberNativeApiQuickJSKnownExposedMethod(
    const std::string& selectorName, const NativeApiQuickJSSignature& signature) {
  if (selectorName.empty()) {
    return;
  }
  NativeApiQuickJSKnownExposedMethod method{
      .selectorName = selectorName,
      .signature = signature,
  };
  std::lock_guard<std::mutex> lock(gNativeApiQuickJSKnownExposedMethodsMutex);
  gNativeApiQuickJSKnownExposedMethods[selectorName] = method;
  gNativeApiQuickJSKnownExposedMethods[jsifySelector(selectorName.c_str())] =
      std::move(method);
}

std::optional<NativeApiQuickJSKnownExposedMethod> knownNativeApiQuickJSExposedMethod(
    const std::string& name) {
  std::lock_guard<std::mutex> lock(gNativeApiQuickJSKnownExposedMethodsMutex);
  auto it = gNativeApiQuickJSKnownExposedMethods.find(name);
  if (it == gNativeApiQuickJSKnownExposedMethods.end()) {
    return std::nullopt;
  }
  NativeApiQuickJSKnownExposedMethod method = it->second;
  prepareEngineMethodSignature(&method.signature);
  return method;
}

std::optional<NativeApiQuickJSClassBuilderRegistration>
findNativeApiQuickJSClassBuilder(id object) {
  Class cls = object != nil ? object_getClass(object) : Nil;
  std::lock_guard<std::mutex> lock(gNativeApiQuickJSClassBuilderMutex);
  while (cls != Nil) {
    auto it = gNativeApiQuickJSClassBuilders.find(cls);
    if (it != gNativeApiQuickJSClassBuilders.end()) {
      return it->second;
    }
...
NSUInteger nativeApiEngineSymbolIteratorCountByEnumerating(
  ...
  auto registration = findNativeApiQuickJSClassBuilder(self);
  ...
  NativeApiQuickJSRuntimeScope runtimeScope(runtime);
...
NativeApiSymbol runtimeSymbolForClass(
    const std::shared_ptr<NativeApiQuickJSBridge>& bridge, Class cls) {
  if (bridge != nullptr) {
...
Class dispatchPrototypeClassForEngineDerivedReceiver(id receiver,
  ...
  if (receiverClass == Nil || receiverClass == prototypeClass ||
      !class_conformsToProtocol(
          receiverClass, @protocol(NativeApiQuickJSClassBuilderProtocol))) {
    return Nil;
  }
...
std::optional<NativeApiQuickJSType> readExposedType(
    Runtime& runtime, const std::shared_ptr<NativeApiQuickJSBridge>& bridge,
...
std::optional<NativeApiQuickJSSignature> exposedMethodSignature(
    Runtime& runtime, const std::shared_ptr<NativeApiQuickJSBridge>& bridge,
...
std::optional<NativeApiQuickJSSignature> runtimeProtocolMethodSignature(
    const char* types) {
  if (types == nullptr) {
    return std::nullopt;
  }
...
void addEngineExposedMethod(Runtime& runtime,
                         ...
                         NativeApiQuickJSSignature signature, Function function) {
  if (selectorName.empty()) {
    return;
  }
  auto callback = createEngineMethodCallback(runtime, bridge, selectorName,
...
Value extendNativeApiQuickJSClass(
    Runtime& runtime, const std::shared_ptr<NativeApiQuickJSBridge>& bridge,
  ...
  markNativeApiQuickJSExtendedClass(nativeClass);
  class_addProtocol(nativeClass, @protocol(NativeApiQuickJSClassBuilderProtocol));
  rememberNativeApiQuickJSClassBuilder(runtime, bridge, nativeClass);
        ...
        addedOverride = true;
	      ...
	      if (!addedOverride) {
	        ...
	        if (!addedRuntimeProtocolOverride) {
	          if (auto known = knownNativeApiQuickJSExposedMethod(propertyName)) {
    ...
    for (size_t i = 0; i < exposedNames.size(runtime); i++) {
      ...
      if (!function) {
        continue;
	      ...
	      if (signature) {
	        rememberNativeApiQuickJSKnownExposedMethod(selectorName, *signature);
	        addEngineExposedMethod(runtime, bridge, nativeClass, selectorName,
	                            std::move(*signature), std::move(*function));
	      }
    }
...
Value invokeNativeApiQuickJSBaseMethod(
    Runtime& runtime, const std::shared_ptr<NativeApiQuickJSBridge>& bridge,
    const Value* args, size_t count) {
