Object CreateNativeApiHermes(Runtime& runtime, const NativeApiHermesConfig& config) {
  auto bridge = std::make_shared<NativeApiHermesBridge>(config);
  return Object::createFromHostObject(
      runtime, std::make_shared<NativeApiHostObject>(std::move(bridge)));
}

void NativeApiHermesWriteSmokeStage(const char* stage) {
...
void InstallNativeApiHermesGlobalSymbols(Runtime& runtime, const char* globalName) {
  NativeApiHermesWriteSmokeStage("engine:globals:before-eval");
...
(function(nativeApiGlobalName) {
  ...
  runtime.evaluateJavaScript(std::make_shared<StringBuffer>(std::move(script)),
                             "NativeApiHermesGlobals.js");
  NativeApiHermesWriteSmokeStage("engine:globals:after-eval");
}

void InstallNativeApiHermes(Runtime& runtime, const NativeApiHermesConfig& config) {
  const char* globalName = config.globalName != nullptr && config.globalName[0] != '\0'
                               ? config.globalName
                               : "__nativeScriptNativeApi";
  NativeApiHermesWriteSmokeStage("engine:create-api");
  Object api = CreateNativeApiHermes(runtime, config);
  Object global = runtime.global();
  NativeApiHermesWriteSmokeStage("engine:set-global");
  global.setProperty(runtime, globalName, api);

  NativeApiHermesWriteSmokeStage("engine:set-interop");
  ...
  if (config.installGlobalSymbols) {
    NativeApiHermesWriteSmokeStage("engine:install-globals");
    InstallNativeApiHermesGlobalSymbols(runtime, globalName);
  } else {
    NativeApiHermesWriteSmokeStage("engine:install-aggregate-globals");
    InstallAggregateGlobals(runtime, api, "protocolNames");
  }
  NativeApiHermesWriteSmokeStage("engine:installed");
}
