...
class NativeApiHermesReturnStorage {
 public:
  explicit NativeApiHermesReturnStorage(size_t size)
...
class NativeApiHermesPointerFrame {
 public:
  explicit NativeApiHermesPointerFrame(size_t count) : count_(count) {
...
Value constantToValue(Runtime& runtime,
  ...
  switch (evalKind) {
    ...
    case metagen::mdEvalString: {
      ...
      NativeApiHermesType stringObjectType;
  ...
  NativeApiHermesType type = parseMetadataEngineType(metadata, &typeOffset, bridge.get());
...
void prepareEngineArgument(Runtime& runtime,
                        const std::shared_ptr<NativeApiHermesBridge>& bridge,
                        const NativeApiHermesType& type, const Value& arg,
                        size_t index, NativeApiHermesArgumentFrame& frame) {
...
inline uint64_t dispatchIdForEngineSignature(
    const NativeApiHermesSignature& signature, SignatureCallKind kind) {
...
struct NativeApiHermesPreparedCFunctionInvocation {
...
bool tryCallFastEngineCFunction(
    ...
    void* function, const NativeApiHermesSignature& signature, const Value* args,
    size_t count, Value* result);

Value callNativeFunctionPointer(
    Runtime& runtime, const std::shared_ptr<NativeApiHermesBridge>& bridge,
    const NativeApiHermesType& type, void* pointer, bool block, const Value* args,
    size_t count) {
  if (pointer == nullptr) {
  ...
  NativeApiHermesArgumentFrame frame(signature->argumentTypes.size());
  prepareEngineArguments(runtime, bridge, *signature, args, count, frame);

  NativeApiHermesPointerFrame values(signature->argumentTypes.size() + 1);
  if (block) {
  ...
  if (block) {
    auto literal = static_cast<NativeApiHermesBlockLiteral*>(pointer);
  ...
  NativeApiHermesReturnStorage returnStorage(
...
Value wrapNativeFunctionPointer(Runtime& runtime,
                                ...
                                bool block) {
  const char* functionName = block ? "NativeApiHermesBlock" : "NativeApiHermesFunctionPointer";
...
Value callCFunction(Runtime& runtime,
                    const std::shared_ptr<NativeApiHermesBridge>& bridge,
                    const std::shared_ptr<NativeApiHermesPreparedCFunctionInvocation>& prepared,
                    const Value* args,
                    size_t count) {
  if (prepared == nullptr) {
  ...
  NativeApiHermesSignature& signature = prepared->signature;
  Value fastResult;
  ...
  NativeApiHermesType returnType = signature.returnType;
  if (retainedReturn) {
...
Value callCFunction(Runtime& runtime,
                    ...
                    size_t count) {
  auto prepared = std::make_shared<NativeApiHermesPreparedCFunctionInvocation>();
  prepared->symbol = symbol;
  return callCFunction(runtime, bridge, prepared, args, count);
}

bool signatureSupportedForEngineInvocation(
    const std::optional<NativeApiHermesSignature>& signature) {
  if (!signature || !signature->prepared || signature->variadic ||
      unsupportedEngineType(signature->returnType)) {
    return false;
  }
...
struct NativeApiHermesPreparedObjCInvocation {
...
bool isFastEngineObjectType(const NativeApiHermesType& type) {
  switch (type.kind) {
...
bool isFastEngineSignedIntegerType(const NativeApiHermesType& type) {
  switch (type.kind) {
...
bool isFastEngineUnsignedIntegerType(const NativeApiHermesType& type) {
  switch (type.kind) {
...
bool readFastEngineObjectArgument(
    Runtime& runtime, const std::shared_ptr<NativeApiHermesBridge>& bridge,
    const NativeApiHermesType& type, const Value& value,
    NativeApiHermesArgumentFrame& frame, id* result) {
  if (result == nullptr) {
    return false;
  }
...
Value callFastEngineCFunctionWithReturn(
    Runtime& runtime, const std::shared_ptr<NativeApiHermesBridge>& bridge,
    void* function, NativeApiHermesType returnType, Args... nativeArgs) {
  bool dispatchingNativeCallToUI = shouldDispatchNativeCallToUI();
  bool retainedReturn = false;

  auto finalizeObjectReturn = [&](id object) -> Value {
    NativeApiHermesType effectiveReturnType = returnType;
...
bool isFastEngineCallableReturnType(const NativeApiHermesType& returnType) {
...
bool tryCallFastEngineCFunction(
  ...
  NativeApiHermesArgumentFrame frame(count);
...
Value callFastEngineObjCWithReturn(
    Runtime& runtime, const std::shared_ptr<NativeApiHermesBridge>& bridge,
    id receiver, SEL selector, NativeApiHermesType returnType,
...
Value callFastEngineObjC1(
    Runtime& runtime, const std::shared_ptr<NativeApiHermesBridge>& bridge,
    id receiver, SEL selector, const NativeApiHermesType& returnType,
...
bool tryCallFastEngineObjCSelector(
    Runtime& runtime, const std::shared_ptr<NativeApiHermesBridge>& bridge,
    id receiver, const NativeApiHermesPreparedObjCInvocation& prepared,
    const Value* args, size_t count, Class dispatchSuperClass, Value* result) {
  if (result == nullptr || receiver == nil || dispatchSuperClass != Nil) {
    return false;
  }

  const NativeApiHermesSignature& signature = prepared.signature;
...
std::shared_ptr<NativeApiHermesPreparedObjCInvocation>
prepareNativeApiHermesObjCInvocation(
    Runtime& runtime, const std::shared_ptr<NativeApiHermesBridge>& bridge,
  ...
  std::optional<NativeApiHermesSignature> signature;
  std::optional<NativeApiHermesSignature> runtimeSignature;
  if (member != nullptr &&
  ...
  auto prepared = std::make_shared<NativeApiHermesPreparedObjCInvocation>();
...
Value callPreparedObjCSelector(
    ...
    const NativeApiHermesPreparedObjCInvocation& prepared, const Value* args,
  ...
  NativeApiHermesPointerFrame values(signature.argumentTypes.size() + 2);
  size_t valueIndex = 0;
...
Value callObjCSelector(Runtime& runtime,
  ...
  NativeApiHermesPreparedObjCInvocation engineInvocation;
