...
static char gNativeApiHermesExtendedClassKey;

void markNativeApiHermesExtendedClass(Class cls) {
  if (cls == Nil) {
    return;
  }
  objc_setAssociatedObject(cls, &gNativeApiHermesExtendedClassKey, @YES,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

bool isNativeApiHermesExtendedClass(Class cls) {
  Class current = cls;
  while (current != Nil) {
    if (objc_getAssociatedObject(current, &gNativeApiHermesExtendedClassKey) != nil) {
      return true;
    }
...
struct NativeApiHermesAggregateInfo;

struct NativeApiHermesFfiType {
  ffi_type type = {};
  std::vector<ffi_type*> elements;

  NativeApiHermesFfiType() {
...
struct NativeApiHermesType {
  ...
  std::shared_ptr<NativeApiHermesType> elementType;
  std::shared_ptr<NativeApiHermesAggregateInfo> aggregateInfo;
  std::shared_ptr<NativeApiHermesFfiType> ownedFfiType;
};

struct NativeApiHermesAggregateField {
  std::string name;
  uint16_t offset = 0;
...
struct NativeApiHermesAggregateInfo {
  ...
  std::vector<NativeApiHermesAggregateField> fields;
  std::shared_ptr<NativeApiHermesFfiType> ffi;
};
...
class NativeApiHermesBridge {
 public:
  explicit NativeApiHermesBridge(const NativeApiHermesConfig& config)
  ...
  std::shared_ptr<NativeApiHermesScheduler> scheduler() const { return scheduler_; }
  ...
  std::shared_ptr<NativeApiHermesAggregateInfo> aggregateInfoFor(
 ...
 private:
  ...
  static std::unique_ptr<MDMetadataReader> loadMetadata(
      const NativeApiHermesConfig& config) {
  ...
  std::shared_ptr<NativeApiHermesScheduler> scheduler_;
  ...
  std::unordered_map<MDSectionOffset, std::shared_ptr<NativeApiHermesAggregateInfo>>
...
NativeApiHermesType nativeObjectReturnType(
...
NativeApiHermesType nativeObjectReturnTypeForClass(Class cls) {
  if (cls != Nil) {
...
NativeApiHermesType primitiveInteropType(MDTypeKind kind);
